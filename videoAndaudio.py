...
def stop_AVrecording(filename):
	...
	print("J'ai arrete le thread video")
...
#	 Merging audio and video signal
	...
	if abs(recorded_fps - FPS) >= 0.01:    # If the fps rate was higher/lower than expected, re-encode it to the expected
		...
		print ("Muxing")
		cmd = "ffmpeg -ac 2 -channel_layout stereo -i temp_audio.wav -i temp_video2.avi -pix_fmt yuv420p " + "user_" + filename + ".avi"
		subprocess.call(cmd, shell=True)
	
	else:
		
		print ("Normal recording\nMuxing")
		cmd = "ffmpeg -ac 2 -channel_layout stereo -i temp_audio.wav -i temp_video.avi -pix_fmt yuv420p " + "user_" + filename + ".avi"
		subprocess.call(cmd, shell=True)
...
def file_manager(filename):
	...
	if os.path.exists(str(local_path) + "/user_" + filename + ".avi"):
		os.remove(str(local_path) + "/user_" + filename + ".avi")
...
def on_key_press(event):
	if event.name == 'enter':
		...
		print("J'AI DETECTE AVOIR APPUYE SUR Q")
...
if __name__== "__main__":
	...
	while(True):
		...
		#Pour que le nom du fichier soit différent à chaque prise
		filename = str(int(filename)+1)
 
		print("Press the Enter key to start the program or the Q key to exit...")
		
		keyboard.wait('enter+q')
